...
OUTPUT_BASE = str(COSINE_OUTPUT / "degradation_new_03_deepface")
...
CONFIDENCE_THRESHOLD = 0.0       # No threshold for images (original script didn't filter by det_score)
...
def extract_image_embedding(image_path):
    """Trích xuất vector embedding từ ảnh sử dụng DeepFace"""
    try:
        path = image_path
...
def process_dataframe(df, view, blur_level):
    ...
    for idx, row in df.iterrows():
        ...
        test_res = extract_image_embedding(test_path)
