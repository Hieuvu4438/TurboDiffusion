...
"""
evaluate_identity_preservation_combined.py
...
OUTPUT_BASE = str(COSINE_OUTPUT / "evaluate_full")
...
#  Các pattern đã quan sát:
#    Aaron_Peirsol_frontal_downup_l1.mp4  → Aaron_Peirsol
#    Abdullah_Gul_side_downup_l1.mp4      → Abdullah_Gul
#    Aaron_Peirsol_poisson_low_light_L1.mp4 → Aaron_Peirsol
# ─────────────────────────────────────────────

def parse_person_from_filename(filename: str, ref_dir: str) -> str | None:
    """
    Trích xuất tên người từ tên file video bằng cách so khớp
    với danh sách ref có sẵn. Đây là cách chắc chắn nhất vì
    tên người có thể chứa dấu gạch dưới không đoán được.
    """
    stem = os.path.splitext(filename)[0]  # bỏ .mp4

    # Lấy danh sách person có trong ref_dir (lấy 1 lần và cache ngoài)
    # Hàm này được gọi với known_persons đã được truyền vào (xem generate_csv)
    return stem  # placeholder, thực tế dùng known_persons bên dưới


def extract_person_name(stem: str, known_persons: list[str]) -> str | None:
    """
...
def extract_image_embedding(image_path: str, app):
    """Trích xuất 512-d embedding từ ảnh ref (chọn mặt lớn nhất)."""
    try:
...
def extract_video_embedding(video_path: str, app):
    """
    Quét toàn bộ video, chọn frame có mặt hướng thẳng nhất
    (det_score > 0.6, tối thiểu hoá |yaw|*1.5 + |pitch| + |roll|*0.5).
    """
    cap = None
    try:
        ...
        while True:
            ...
            err = abs(yaw) * 1.5 + abs(pitch) + abs(roll) * 0.5
            if err < best_err:
                best_err = err
        ...
        if best_emb is not None:
            ...
            return {"emb": None, "err": "No suitable face (det_score > 0.6)", "image": None}
    except Exception as e:
...
def process_entry(entry: dict, app, df: pd.DataFrame):
    """
    ...
    print(f"  Extracting embeddings ({len(df)} videos)...")
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate face identity preservation (cosine similarity)")
    parser.add_argument(
        "--device",
        choices=["gpu", "cpu"],
        default="gpu",
        help="Thiết bị chạy InsightFace: 'gpu' (mặc định) hoặc 'cpu'",
    )
    parser.add_argument(
        "--skip_existing",
        action="store_true",
    ...
    if args.device == "gpu":
        ...
        ctx_id = 0
        print("[Device] GPU (CUDAExecutionProvider)")
    else:
        providers = ['CPUExecutionProvider']
        ctx_id = -1
        print("[Device] CPU (CPUExecutionProvider)")
    ...
    for entry in entries:
        ...
        # Luôn generate lại từ file .mp4 (CSV của lowlight/frontal đã có sẵn
        # trong input cũng được generate lại cho đồng nhất format)
