"""
Batch Face Restoration: Run inference on jpeg and salt-pepper degraded CSV files for side view.

This script finds all CSV files under
    degradation_experiment/side/{deg_type}/{level}/
for deg_type in {jpeg, salt-pepper} and runs run_face_restoration.py on each one
sequentially.

Usage:
    python scripts/run_jpeg_saltpepper_restoration_side.py
    python scripts/run_jpeg_saltpepper_restoration_side.py --dry_run
    python scripts/run_jpeg_saltpepper_restoration_side.py --skip_existing
    python scripts/run_jpeg_saltpepper_restoration_side.py --deg_types jpeg
    python scripts/run_jpeg_saltpepper_restoration_side.py --levels L1 L2
"""
...
def run_restoration_for_csv(csv_info: dict, args: argparse.Namespace) -> dict:
    ...
    output_dir = PROJECT_ROOT / "output_side_new_combined" / deg_type / level
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run face restoration on jpeg / salt-pepper degraded CSV files (SIDE VIEW)",
    ...
    print("=" * 70)
    print("  🚀 Face Restoration (SIDE) — jpeg / salt-pepper Batch Run")
    print("=" * 70)
    ...
    for deg_type in deg_types:
        for level in levels:
            out_dir = PROJECT_ROOT / "output_side_new_combined" / deg_type / level
            print(f"    {out_dir}/")
    print(f"{'='*70}")
