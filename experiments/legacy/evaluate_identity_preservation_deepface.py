...
OUTPUT_BASE = "/home/haipd/TurboDiffusion/cosine_similarity_output/i2v_new_03_deepface"

# DeepFace model and detector configuration
MODEL_NAME = "ArcFace"       # ArcFace produces 512-d embeddings (same as InsightFace buffalo_l)
DETECTOR_BACKEND = "retinaface"  # RetinaFace for accurate detection + landmarks
CONFIDENCE_THRESHOLD = 0.6  # Equivalent to InsightFace det_score > 0.6


def estimate_frontality_error(facial_area):
    """
    Estimate a frontality error score from facial landmarks.
    Lower error = more frontal face.
    
    This mimics InsightFace's pose-based selection:
      error = abs(yaw) * 1.5 + abs(pitch) + abs(roll) * 0.5
    
    We use landmark symmetry as a proxy:
    - Yaw proxy: how off-center the nose is between the two eyes
    - Roll proxy: angle of the eye-line relative to horizontal
    """
    landmarks = {}
    for key in ['left_eye', 'right_eye', 'nose', 'mouth_left', 'mouth_right']:
        if key in facial_area and facial_area[key] is not None:
            landmarks[key] = facial_area[key]
    
    # Need at least eyes and nose for estimation
    if 'left_eye' not in landmarks or 'right_eye' not in landmarks or 'nose' not in landmarks:
        return 50.0  # High penalty if landmarks missing
    ...
    # Yaw proxy: horizontal offset of nose from eye midpoint, normalized by eye distance
    # A perfectly frontal face has nose directly below eye midpoint
    yaw_proxy = abs(nose[0] - eye_mid[0]) / eye_dist
    
    # Roll proxy: angle of eye line relative to horizontal
    dy = re[1] - le[1]
    dx = re[0] - le[0]
    roll_proxy = abs(math.degrees(math.atan2(dy, dx)))
    
    # Pitch proxy: vertical offset of nose from eye midpoint, normalized
    # For frontal faces, nose should be below eyes at a typical ratio
    pitch_proxy = abs((nose[1] - eye_mid[1]) / eye_dist - 0.65)  # 0.65 is typical ratio
    
    # Combine similar to original: error = abs(yaw)*1.5 + abs(pitch) + abs(roll)*0.5
    error = yaw_proxy * 1.5 * 30 + pitch_proxy * 30 + roll_proxy * 0.5
    
    return error


def normalize_embedding(emb):
    """Normalize embedding to unit vector (L2 normalization) like InsightFace's normed_embedding"""
...
def extract_image_embedding(image_path):
    """Trích xuất vector embedding từ ảnh gốc sử dụng DeepFace"""
    try:
        ...
        # Chọn mặt lớn nhất (equivalent to InsightFace sorting by bbox area)
        best_face = max(results, key=lambda f: f['facial_area']['w'] * f['facial_area']['h'])
        
        # Check confidence
        confidence = best_face.get('face_confidence', 0)
        if confidence < CONFIDENCE_THRESHOLD:
            return {"emb": None, "err": f"Low confidence: {confidence:.2f}", "image": None}
...
def extract_video_embedding(video_path):
    """Trích xuất vector embedding từ video (quét toàn bộ video) sử dụng DeepFace"""
    try:
        ...
        while True:
            ...
            if len(results) > 0:
                # Lọc mặt lớn nhất
                face = max(results, key=lambda f: f['facial_area']['w'] * f['facial_area']['h'])
                
                # Điều kiện confidence > 0.6 (equivalent to det_score > 0.6)
                confidence = face.get('face_confidence', 0)
                if confidence > CONFIDENCE_THRESHOLD:
                    # Estimate frontality error from landmarks
                    error = estimate_frontality_error(face['facial_area'])
                    
                    if error < best_err:
                        best_err = error
                        best_emb = normalize_embedding(face['embedding'])
                        best_img = frame.copy()
        ...
        if best_emb is not None:
            ...
            return {"emb": None, "err": "No suitable face found (confidence > 0.6)", "image": None}
...
def calculate_cosine_similarity(emb1, emb2):
    """Tính cosine similarity, trả về np.nan nếu có lỗi"""
    try:
...
def generate_report(out_dir, view, blur_level, total_persons, df_pairs):
    ...
    lines = [
        ...
        f"  Model: DeepFace ({MODEL_NAME}) | Detector: {DETECTOR_BACKEND}",
        "======================================================================",
        "",
...
def process_dataframe(df, view, blur_level):
    ...
    for idx, row in df.iterrows():
        ...
        vid_res = extract_video_embedding(video_path)
        ref_res = extract_image_embedding(ref_path)
...
def main():
    print(f"Loading DeepFace model: {MODEL_NAME} with detector: {DETECTOR_BACKEND}...")
    
    # Warm up the model by running a dummy represent call
    # This pre-loads the model weights into GPU memory
    dummy_img = np.zeros((224, 224, 3), dtype=np.uint8)
    try:
        DeepFace.represent(
            img_path=dummy_img,
            model_name=MODEL_NAME,
            detector_backend="skip",  # skip detection for dummy warmup
            enforce_detection=False
        )
        print("Model loaded and warmed up successfully.")
    except Exception as e:
        print(f"Warmup note: {e} (this is normal for blank images)")
    ...
    for view, blur_level in combinations:
        ...
        if dfs:
            ...
            process_dataframe(combined_df, view, blur_level)
