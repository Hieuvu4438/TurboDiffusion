...
OUTPUT_BASE = "/home/haipd/TurboDiffusion/cosine_similarity_output/lowlight_side_frontal_evaluation"
VIDEO_BASE_DIR = "/home/haipd/TurboDiffusion/output_full_new_combined/lowlight/side/frontal"
REF_BASE_DIR = "/home/haipd/TurboDiffusion/Experiment_Data_Split_Combined"
...
def calculate_cosine_similarity(emb1, emb2):
    """Tính toán cosine similarity, trả về np.nan nếu có lỗi"""
    try:
...
def prepare_csv(level):
    level_dir = os.path.join(VIDEO_BASE_DIR, level)
    if not os.path.exists(level_dir):
        print(f"Directory {level_dir} does not exist.")
        return None
        
    videos = glob.glob(os.path.join(level_dir, "*.mp4"))
    data = []
    
    for video in videos:
        basename = os.path.basename(video)
        
        idx = basename.find("_poisson")
        if idx != -1:
            person = basename[:idx]
        else:
            person = os.path.splitext(basename)[0]
            
        # Find ref
        ref_path = ""
        for exp in ["Frontal_Exp", "Side_Exp"]:
            p = os.path.join(REF_BASE_DIR, exp, "Ref", f"{person}_ref.jpg")
            if os.path.exists(p):
                ref_path = p
                break
                
        if not ref_path:
            # Fallback path if it doesn't exist to at least maintain the format
            ref_path = os.path.join(REF_BASE_DIR, "Frontal_Exp", "Ref", f"{person}_ref.jpg")
    ...
    if len(data) == 0:
        print(f"No videos found in {level_dir}")
        return None
        
    df = pd.DataFrame(data)
    csv_path = os.path.join(level_dir, f"{level}_evaluation_data.csv")
...
def main():
    ...
    levels = ["L1", "L2", "L3"]
    for level in levels:
        df = prepare_csv(level)
        if df is not None and not df.empty:
            process_dataframe(df, "lowlight_side_frontal", level, app)
